...
class Simulator():
	...
	def addContainerList(self, containerInfoList):# 添加一组新的容器到模拟器中
		deployed = containerInfoList[:min(len(containerInfoList), self.containerlimit-self.getNumActiveContainers())]# 根据限制添加容器的数量，选择要部署的容器
		deployedContainers = []
		for CreationID, CreationInterval, IPSModel, RAMModel, DiskModel in deployed:
	...
	def destroyCompletedContainers(self):# 销毁已经完成任务的容器
		destroyed = []
	...
	def addContainers(self, newContainerList):
		self.interval += 1
		destroyed = self.destroyCompletedContainers()# 销毁已经完成任务的容器
		deployed = self.addContainerList(newContainerList)# 添加一组新的容器到模拟器中
	...
	def simulationStep(self, decision):
		routerBwToEach = self.totalbw / len(decision) if len(decision) > 0 else self.totalbw# 带宽分配
		migrations = []
		...
		for (cid, hid) in decision:
			...
			allocbw = min(targetHost.bwCap.downlink / migrateToNum, currentHost.bwCap.uplink / migrateFromNum, routerBwToEach)# 计算带宽
			if hid != self.containerlist[cid].hostid and self.getPlacementPossible(cid, hid):# 根据容器的需求和主机的资源判断是否可以迁移
				migrations.append((cid, hid))
				container.allocateAndExecute(hid, allocbw)
				containerIDsAllocated.append(cid)
		# destroy pointer to unallocated containers as book-keeping is done by workload model
		for (cid, hid) in decision:# 清理未分配的容器
