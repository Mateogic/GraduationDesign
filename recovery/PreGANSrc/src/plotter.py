...
class GAN_Plotter():
    ...
    def plot_test(self, hosts_from, epoch):
        ...
        if epoch < 20: return
        self.plot_heatmap('Fault Prediction and Classification', 'Prediction', 'Classification',
                        np.array(self.anomaly_detected).reshape(1, -1), np.array(self.class_detected))
        self.plot_heatmapc('Migrations', 'Migration', 'Hosts from Migration', np.array(self.migrating).reshape(1, -1),
                        np.array(self.hosts_migrated))
        self.plot_heatmap_combined('Combined', np.array(self.anomaly_detected).reshape(1, -1),
                                np.array(self.class_detected), np.array(self.migrating).reshape(1, -1),
                                np.array(self.hosts_migrated))
    ...
    def plot_heatmap(self, title, name1, name2, data1, data2):
        ...
        ax1.set_title(title)
        # 生成一个从0到self.n_hosts的等间距整数数组，并将其转换为int32类型
        yticks = np.linspace(0, self.n_hosts, 2, dtype=np.int32)# 数量为2的等间距数组
        ...
        two_tone_cmap = LinearSegmentedColormap.from_list('TwoToneYlGnBu',
                                                        ...
                                                        (1, high_color)],
                                                        N=256)
        data2 = (data2 + 1).transpose()# -1 0 1 2 -> 0 1 2 3
        # 颜色映射的分段数为4
        ...
        # vmin：定义颜色映射的最小值。数据中小于或等于这个值的部分将映射到颜色条的最小颜色。
        # vmax：定义颜色映射的最大值。数据中大于或等于这个值的部分将映射到颜色条的最大颜色。
        ...
        sm2 = plt.cm.ScalarMappable(cmap=dcmap, norm=plt.Normalize(vmin=0, vmax=4))# 设置颜色条(colorbar)的刻度范围
